...
export const deleteCustomer = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await customerService.delete(req.params['id'] as string, deactivate);
  res.json(ApiResponse.success(null, result.action === 'deactivated' ? 'Customer deactivated successfully' : 'Customer deleted successfully'));
});
