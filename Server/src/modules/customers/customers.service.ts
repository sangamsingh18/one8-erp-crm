...
export const customerService = {
  ...
  async delete(id: string, deactivate = false) {
    await this.getById(id);

    // Check if the customer has any related challans
    const checkRes = await pool.query(
      `SELECT COUNT(*) FROM challans WHERE customer_id = $1`,
      [id]
    );
    ...
    if (count > 0) {
      if (deactivate) {
        await pool.query(
          `UPDATE customers SET status = 'inactive', updated_at = NOW() WHERE id = $1`,
          [id]
        );
        return { action: 'deactivated' };
      } else {
        throw new ApiError(409, 'This customer has existing business records. Deactivating the customer will preserve historical records.');
      }
    }
